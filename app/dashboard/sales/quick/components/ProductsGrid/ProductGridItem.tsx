...
const ProductGridItem = ({ product }: Props) => {
    ...
    const handleVariantSelect = (event: any, variant: ProductInterface) => {
        event.stopPropagation();
        if (inCart(variant.id)) removeItem(variant.id);
        addItem(variant);
    };
    ...
    return (
        <div
            onClick={onAddToCart}
            className={`border relative border-gray-300 bg-white cursor-pointer shadow-md ${
                inCart(product?.id ?? "") ? "shadow-lg shadow-[#615cf6]" : ""
            } hover:shadow-[#615cf6] transition-all duration-500 ease-in-out rounded-2xl p-4 flex flex-col`}
        >
            ...
            {product?.variants?.length && showVariants ? (
                <div className="absolute left-0 right-0 top-full z-10 border border-gray-300 bg-white shadow-md rounded-2xl flex flex-col gap-1 p-2 mb-4">
                    <div
                        onClick={(e) => handleVariantSelect(e, product)}
                    ...
                    {product?.variants?.map((variant) => (
                        ...
                        >
                            <p>
                                ...
                                :{" "}
                                {variant?.variation_meta?.value?.toLocaleUpperCase()}
                            </p>
                            <p>
                                Price:{" "}
                                {variant?.min_selling_price &&
                                variant?.max_selling_price
                                    ? `৳${variant?.min_selling_price} - ৳${variant?.max_selling_price}`
                                    : "৳" +
                                      (variant?.min_selling_price ??
                                          variant?.max_selling_price ??
                                          "0")}
