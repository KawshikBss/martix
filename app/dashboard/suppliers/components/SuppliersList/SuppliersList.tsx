...
export interface ISuppliersListProps {
  data: IProduct[];
}

export function SuppliersList(props: ISuppliersListProps) {
  return (
    <div className="md:hidden mt-4">
      {props.data.length === 0 && <p>No suppliers found.</p>}
      {props.data.length > 0 && (
        <>
          <span className="text-center text-gray-500">
            Showing {props.data.length} suppliers
          </span>
          <div className="space-y-4 mt-4">
            {props.data.map((product) => (
              <SuppliersListItem key={product.id} product={product} />
            ))}
          </div>
